...
- **코딩 스타일**: `docs/guide.md` 참조
  - SOLID 원칙 + 선언적(Declarative) + 불필요한 추상화 금지
  - 네이밍 규칙: 컴포넌트는 PascalCase (`TourCard`, `TourList`)
  - Export 규칙: 단일 컴포넌트는 `export default`, 다중은 named export
  - Spacing-First 정책: 외곽 여백은 padding, 형제 요소 간 간격은 gap
  - Tailwind CSS 우선 사용, 인라인 style 금지
  - Next.js 15 특별 고려사항: 동적 라우트 파라미터는 `await params` 사용
...
- **Tailwind CSS 우선 사용**: 인라인 `style={{ }}` 사용 금지
- **Spacing-First 정책**:
  - 외곽 여백은 최상단 래퍼의 `padding` 사용
  - 형제 요소 간 간격은 부모의 `gap` 사용
  - `margin` 사용 금지 (mt, mb, mx, my)
- **배경 이미지 처리**: Next.js `Image` 컴포넌트 + 오버레이 div 사용
- **그라디언트**: Tailwind 클래스 사용 (`bg-gradient-to-*`)

### 7.3 컬러 스킴

- 다크/라이트 모드 지원
- Primary 색상: 관광/여행 느낌 (청록색, 파란색 계열 추천)
- 디자인 시스템 컬러 팔레트 사용 (하드코딩된 hex 컬러 금지)

### 7.4 로딩 상태

- 리스트 로딩: 스켈레톤 UI
- 지도 로딩: 스피너
- 이미지 로딩: Placeholder 이미지 (Next.js `Image` 컴포넌트 사용)
